...
struct InstanceStat
{
    ...
    InstanceStat() { incStat<InstanceCreated>(); }
    ~InstanceStat() { incStat<InstanceDestroyed>(); }

    static uint64_t createCounter() { return stat<InstanceCreated>().load(std::memory_order_relaxed); }
    static uint64_t destroyCounter() { return stat<InstanceDestroyed>().load(std::memory_order_relaxed); }
};
